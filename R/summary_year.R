...
summary_year.wos.jcr <- function(object, filter, all = FALSE, ...) {
  ...
  n.jcr <- docjcrcat %>%
    summarise(
      ...
      nQ1 = sum(JIFP > 75, na.rm = TRUE), # Documents in Q1
      nD1 = sum(JIFP > 90, na.rm = TRUE), # Documents in D1
      nbest3 = sum(WCP <= 3, na.rm = TRUE), # Documents in top 3 journals
      .by = PY
    ) %>%
    arrange(PY)
  # Result
...
plot.summary.year.jcr <- function(x, plot = TRUE, all = FALSE,
  ...
  pobj <- x$mean.jcr %>%
    tidyr::pivot_longer(JIF:WCP, names_to = "var", values_to = "y") %>%
  ...
  result <- c(result, list(pobj))
  pobj <- x$n.jcr %>%
    tidyr::pivot_longer(ndocjcr:nbest3, names_to = "var", values_to = "y") %>%
