...
# PENDIENTE:
#   Renombrar TC.index
#   Renombrar TC.authors y hacer version simplificada de summary
# ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
...
# ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
  ...
  # object=db; filtered=FALSE; TC.index = c("H", "G"); nmax = 10; docs <- object$Docs
  ...
  status <- with(docs, c("Highly Cited" = sum(HC),
                      "Hot Papers" = sum(HP)))
  # TOP
  ...
  int.col <- aux %>% dplyr::filter(Country != dimnames(top.coun)[[1]][1]) %>%
             pull(idd) %>% unique %>% length
  ...
  # Result
  result <- list(ndocs = nrow(docs), naut = length(autdoc),
...
summary.wos.jcr <- function(object, filter, all = FALSE, ...) {
  ...
  summary.jcr <- docjcrcat %>% select(JIF:JAI, JIFP, WCP) %>% sapply(summary)
  # Artículos en JCR
  ...
  # Result
  result <- list(docjcrcat = docjcrcat,
