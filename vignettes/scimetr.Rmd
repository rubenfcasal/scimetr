...
<!-- 
NOTA:
Javier, puedes decir que estamos "a punto" de sacar la versión 2.0, 
cambiando los nombres de la funciones para homogeneizarlos 
empleando estilo_serpiente (tidyverse) con minúsculas:
ImportSources.wos, CreateDB -> import_sources_wos, create_db
get.idXXXX -> get_id_xxxx
TC.authors -> author_metrics 
-->
...
We will focus exclusively on importing publication data from [WoS](http://www.webofknowledge.com) in text format.
First, you need to download the corresponding files from the WoS website, for example, by following the steps described [here](https://rubenfcasal.github.io/scimetr/articles/docs/WoS_export.html).
...
To create the [bibliographic database](https://en.wikipedia.org/wiki/Bibliographic_database), use the `CreateDB()` function (the result is a `wos.db`-class S3 object):

```{r wosdf, cache=!rebuid}
...
Pendiente:
  ...
  - Añadir tabla dinámica con todas las variables
-->
